		     ...
		     <el-card :body-style="{ padding: '0px' }"  >
			   ...
			   <div  @click="insurance(videodata.list[index].videoId)">
				   ...
				   <div style="padding: 14px;" >
					 <span>{{videodata.list[index].videoName}}</span>
					<span >{{videodata.list[index].videoId}}</span>
					<!-- <span>{{index}}</span> -->
				   </div>
			   </div>
		     </el-card>
		   ...
		   <el-col :span="12" v-if="(index+1)<videodata.list.length">
		     <el-card :body-style="{ padding: '0px' }">
				 <div  @click="insurance(videodata.list[index+1].videoId)">
				   ...
				   <div style="padding: 14px;">
					 <span>{{videodata.list[index+1].videoName}}</span>
					 <span>{{videodata.list[index+1].videoId}}</span>
				   </div>
			   </div>
		     </el-card>
		   </el-col>
...
export default {
  ...
  methods:{
        getData(currentpage){
			console.log("hhhhhhh:"+this.$route.params.search);
          //请求数据
          var api = '/api/getSearchVideo?videoSearch='+this.$route.params.search+'&pageNum='+currentpage;
